...
void cam_2_obj_center(PCT::Ptr i_cloud,
  ...
  float mean_diff = sum_diff / cloud_normal->size();
  ...
  extract.setInputCloud (cloud_normal);
  extract.setIndices (pointIndices);
  extract.setNegative (false);
  extract.filter (*cloud_normal);


  std::cout << "want_below_diff size = " << cloud_normal->size() << std::endl;
  

  obj_normal = get_normal_mean(cloud_normal);
