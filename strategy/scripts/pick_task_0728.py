...
from get_obj_info import order_boxes, info_dict, suck_list
...
class PickTask:
    ...
    def get_all_item_dis_done(self, state, result):
        if result.success:
            self.item_dis[result.object_name] = result.object_pose.linear.z
            self.detected_cnt -= 1

        if not self.detected_cnt:
            self.next_step = VisionProcess2
    ...
    def tool_2_obj_bin(self, obj_pose, norm, rel_pos = (0, 0, -0.2), rel_ang = (10, 0 , 0)): # BIN
        ...
        if self.bin == "e" or self.bin == "f" :
            ...
            gripper_suction_deg(90)
            r = 90
            self.Arm.relative_xyz_base(x = real_move_z_rot, y = real_move_x_rot, z = -0.02, blocking=True)
        else :
            self.Arm.relative_xyz_base(x = real_move_z_rot, y = real_move_x_rot, z = real_move_y_rot*-1 + 0.06, blocking=True)
