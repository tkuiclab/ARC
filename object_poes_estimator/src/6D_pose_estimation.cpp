...
void ObjEstAction::estimate_pose(){
    // Point clouds
...
void ObjEstAction::get_roi(){
  ...
  pcl::console::print_highlight ("Loading point clouds...\n");
  if (pcl::io::loadPCDFile<PointNT> ("/home/iclab-ming/ARC_ws/src/object_poes_estimator/pcd_file/Scene_with_handweight.pcd", *scene) < 0)
  {
    pcl::console::print_error ("Error loading object/scene file!\n");
  }
  ...
  roi_srv.request.object_name = goal_;
...
{
  state = foto;
  goal_ = as_.acceptNewGoal()->object_name;
  ROS_INFO("Action calling! Goal=%s",goal_.c_str());    
}
